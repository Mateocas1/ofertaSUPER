...
export type RuntimeContractResult = { role: RuntimeRole; missing: string[]; invalid: string[] };
...
const OPTIONAL_PAIRS = [["UPSTASH_REDIS_REST_URL", "UPSTASH_REDIS_REST_TOKEN"]] as const;
...
export function validateRuntimeContract(role: RuntimeRole, env: RuntimeEnvironment): RuntimeContractResult {
	...
	for (const [first, second] of OPTIONAL_PAIRS) {
		if (isPresent(env[first]) !== isPresent(env[second])) required.push(isPresent(env[first]) ? second : first);
	}
