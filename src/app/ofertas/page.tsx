...
export const metadata: Metadata = createMetadata({
  ...
  description: "Promociones activas y productos con descuento detectado en supermercados argentinos.",
  path: "/ofertas",
});
...
export default async function OffersPage({ searchParams }: OffersPageProps) {
  ...
  const type = getSingleParam(params.type) as "2x1" | "2nd_50" | "wallet_discount" | "bank_discount" | "percentage" | undefined;

  const [promotions, discountedProducts] = await Promise.all([
    getPromotions({ supermarket, wallet, type }),
    listProducts({ supermarket, offersOnly: true, sort: "discount", limit: 24, page: 1 }),
  ]);
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-8">
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-lg text-muted-foreground">
                Cruza promociones activas de bancos y billeteras con descuentos detectados automaticamente por diferencia entre list price y precio actual.
              </p>
            </div>
        ...
        <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="surface p-6">
            <div>
              <p className="text-sm uppercase tracking-[0.18em] text-muted-foreground">Promociones activas</p>
              <h2 className="mt-2 text-3xl font-semibold text-foreground">Carga manual administrable</h2>
            </div>

            <div className="mt-6 space-y-4">
              {promotions.map((promotion) => (
                <article key={promotion.id} className="rounded-[1.5rem] border border-border/70 bg-white/80 p-5">
                  ...
                  <h3 className="mt-4 text-xl font-semibold text-foreground">{promotion.title}</h3>
                  <p className="mt-2 text-sm leading-6 text-muted-foreground">
                    {[promotion.walletProvider, promotion.bankName, promotion.conditions].filter(Boolean).join(" • ") || "Sin condiciones registradas"}
                  </p>
                </article>
              ))}
          ...
          <div className="space-y-5">
            <div>
              <p className="text-sm uppercase tracking-[0.18em] text-muted-foreground">Descuento automatico</p>
              <h2 className="mt-2 text-3xl font-semibold text-foreground">Productos con list price por encima del precio real</h2>
            </div>
