...
export default async function CategoryPage({ params, searchParams }: CategoryPageProps) {
  ...
  const filters = filtersFrom(rawSearchParams);
  const page = await loadCategoryPage(category.name, filters.supermarket, filters.sort, filters.maxPrice, filters.offers);
  const result = page.availability === "eligible" ? page.catalog : null;
  ...
  return (
    <div className="px-6 py-8 md:py-10">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-8">
        ...
        {page.availability === "unavailable" ? <section role="alert" className="rounded-[1.5rem] border border-amber-300 bg-amber-50 px-5 py-4 text-sm leading-6 text-amber-950"><p className="font-semibold">El catálogo no está disponible.</p><p className="mt-1">No podemos mostrar productos reales en este momento. Probá de nuevo más tarde.</p></section> : null}
        {allResultsAreStale ? <StaleResultsNotice context="productos de esta categoría" /> : null}
        {result ? <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">{result.items.map((product) => <ProductCard key={product.ean} product={product} />)}</section> : null}
        {result && result.items.length === 0 ? <section className="surface-soft p-6 text-sm text-muted-foreground">No hay resultados con esos filtros dentro de {category.name}.</section> : null}
      </div>
    </div>
  );
}
