...
describe("approved home visual slice content", () => {
	it("keeps the approved navigation, hero and signal copy focused on search first", () => {
		...
		assert.equal(
			HOME_HERO.heading,
			"Compará precios. Armá tu canasta. Comprá mejor.",
		);
		assert.equal(
			HOME_HERO.body,
		...
		assert.equal(
			...
			"Buscar leche, yerba, arroz, aceite...",
		);
	...
	it("keeps the smart basket and product preview within the first approved slice", () => {
		...
		assert.equal(
			SMART_BASKET.ranking.some((item) => item.status === "Falta 1"),
			true,
		);
		assert.equal(HOME_PRODUCT_ROWS.length, 4);
		assert.equal(
			HOME_PRODUCT_ROWS.every((row) => row.action === "Agregar"),
			true,
		);
	...
	it("keeps public home copy free from portfolio red-flag terms", () => {
		const copy = getApprovedHomeCopy().join(" ");
		const updatedAtLabels: string[] = HOME_PRODUCT_ROWS.map(
			(row) => row.updatedAt,
		);
		...
		assert.deepEqual(
			updatedAtLabels,
			Array(HOME_PRODUCT_ROWS.length).fill("Último registro disponible"),
		);
		assert.match(copy, /Lecturas del catálogo/);
	});
});
