...
function repository(extraRows: DirectRefreshExistingRow[] = []) {
	...
	return {
		...
		async findRowsBySourceEan(sourceSlug: string, ean: string) {
			return allRows.filter((row) => row.sourceSlug === sourceSlug && row.ean === ean);
		},
	};
}
...
describe("direct refresh feasibility", () => {
	it("classifies URL, SKU and unique EAN probes without writes", async () => {
		...
		const report = await buildDirectRefreshFeasibilityReport({
			...
			fetchProducts: async (_sourceSlug, term) => {
				...
				return (
					{
						"https://www.vea.com.ar/leche/p": [
							{ ean: "111", skuId: null, productUrl: "https://www.vea.com.ar/leche/p?utm=1" },
						],
						"sku-222": [{ ean: "222", skuId: "sku-222", productUrl: null }],
						"333": [{ ean: "333", skuId: null, productUrl: null }],
					}[term] ?? []
				);
			},
		});
	...
	it("distinguishes ambiguous candidates, source drift, collisions, and SKU mismatch", async () => {
		const report = await buildDirectRefreshFeasibilityReport({
			repository: repository([
				{ id: "ambiguous-2", sourceSlug: "dia", ean: "444", skuId: null, productUrl: null },
	...
	it("fails closed when every sampled row is unviable and keeps CLI read-only", async () => {
		const report = await buildDirectRefreshFeasibilityReport({
			repository: {
				...repository(),
				async listSources() {
				...
				async listOldestPublicRankableRows() {
					return [{ id: "x1", sourceSlug: "x", ean: null, skuId: null, productUrl: null }];
				},
			},
			fetchProducts: async () => [],
		});
		...
		assert.deepEqual(
			parseDirectRefreshFeasibilityCliOptions(["node", "script", "--source=vea,dia", "--sample-size=2", "--count=3", "--output=a0-direct.json"]),
			{ sources: ["vea", "dia"], sampleSize: 2, count: 3, output: "a0-direct.json" },
		);
		assert.throws(
			() => parseDirectRefreshFeasibilityCliOptions(["node", "script", "--confirm-write"]),
			/read-only/,
		);
	});
});
