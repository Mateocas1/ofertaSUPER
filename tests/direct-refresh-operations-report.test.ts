...
const passArtifacts = {
	...
	alerts: {
		...
		status: "PASS",
		summary: { criticalCount: 0, highCount: 0, mediumCount: 0, lowCount: 0, highestSeverity: null },
		alerts: [],
	},
	killSwitch: {
		audit: "direct-refresh-kill-switch",
		status: "PASS",
		summary: { activeStopCount: 0, expiredStopCount: 0, invalidControlCount: 0 },
		activeControls: [],
		invalidControls: [],
	},
	freshnessBaseline: {
...
describe("direct-refresh operations report", () => {
	...
	it("preserves top-level source health WARN when individual sources pass", () => {
		const report = buildDirectRefreshOperationsReport({
			...
			artifacts: {
				sourceHealth: {
					...
					status: "WARN",
					summary: { writerSupportedStatus: "PASS" },
					sources: [{ slug: "vea", directRefreshSupport: "writer-supported", status: "PASS", reasons: [] }],
				},
			},
		});
		...
		assert.match(report.blockedSources[0].reason, /source health report status is WARN/);
	});
	...
	it("preserves top-level alert and kill-switch failures when child arrays are empty", () => {
		...
		assert.match(report.blockedSources.map((entry) => entry.reason).join("\n"), /alerts report status is FAIL/);
		...
		const report = buildDirectRefreshOperationsReport({
			...
			artifacts: {
				sourceHealth: {
					...
					summary: { writerSupportedStatus: "PASS" },
					sources: [
						{ slug: "mas", directRefreshSupport: "writer-supported", status: "WARN", reasons: ["freshness below target"], freshness: { freshnessPercent: 80 }, capacity: { blockedRows: 5 } },
						{ slug: "dia", directRefreshSupport: "audit-only-no-writer", status: "WARN", reasons: ["source is audit-only/no-writer"] },
					],
				},
			},
		});
		...
		const report = buildDirectRefreshOperationsReport({
			...
			artifacts: {
				alerts: {
					audit: "direct-refresh-alerts",
					status: "FAIL",
					summary: { criticalCount: 0, highCount: 1, mediumCount: 0, lowCount: 0, highestSeverity: "HIGH" },
					alerts: [{ condition: "source_health_fail", severity: "HIGH", source: "vea", message: "Source health FAIL", nextAction: "Fix source" }],
				},
				killSwitch: {
					audit: "direct-refresh-kill-switch",
					status: "FAIL",
					summary: { expiredStopCount: 0 },
					activeControls: [{ scope: "source", source: "vea", reason: "incident", owner: "Maintainer" }],
					invalidControls: [],
				},
			},
		});
	...
	it("maps baseline and operation failures into blockers", () => {
		const report = buildDirectRefreshOperationsReport({
			now,
			artifacts: {
				freshnessBaseline: { audit: "freshness-baseline", status: "FAIL", summary: { overallFreshnessPercent: 50 }, denominatorDeltas: { status: "FAIL", blockers: ["denominator shrank"] } },
				manifest: { audit: "vea-direct-refresh-manifest-dry-run", status: "FAIL", source: { slug: "vea" }, summary: { failClosedReasons: ["no rows selected"], skippedBlockedReasons: { "live product unavailable": 5 } } },
				postwrite: { audit: "vea-direct-refresh-postwrite-audit", status: "FAIL", writeReport: { source: "vea" }, noCreate: { productDelta: 1, supermarketProductDelta: 0 }, summary: { failClosedReasons: ["row failed"] } },
			},
		});
	...
	it("handles missing optional and required artifacts", () => {
		...
		assert.match(required.blockedSources.map((entry) => entry.reason).join("\n"), /manifest artifact is required/);
	...
	it("requires no-partial verification for incident artifacts by default", () => {
		const missing = buildDirectRefreshOperationsReport({
			now,
			artifacts: { errorArtifact: { artifact: "direct-refresh-error", source: "mas", error: "timeout" } },
		});
		...
		const verified = buildDirectRefreshOperationsReport({
			now,
			artifacts: { errorArtifact: { artifact: "direct-refresh-error", source: "mas", error: "timeout" }, noPartial: { audit: "direct-refresh-no-partial-verification", source: "mas", conclusion: "PASS" } },
		});
	...
	it("parses CLI allowed inputs and rejects unsafe flags", () => {
		...
		for (const flag of ["--write", "--confirm-write=1", "--all-source", "--scheduler=true", "--notify", "--notifications=true", "--deploy", "--refresh"]) {
			assert.throws(
				() => parseDirectRefreshOperationsReportCliOptions(["node", "script", flag]),
	...
	it("rejects unknown, bare, and invalid boolean flags", () => {
		assert.throws(
			() => parseDirectRefreshOperationsReportCliOptions(["node", "script", "--dry-run"]),
			/unknown direct-refresh operations report flag/,
		);
		assert.throws(
			() => parseDirectRefreshOperationsReportCliOptions(["node", "script", "--source"]),
			/requires --source=\.\.\./,
		);
		assert.throws(
			() => parseDirectRefreshOperationsReportCliOptions(["node", "script", "--require-postwrite=yes"]),
