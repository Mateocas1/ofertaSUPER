...
- [x] **U14 — Forward corrective generation** — U14a and U14b are complete.
  ...
  - [x] **U14b — Durable guarded correction lifecycle**
    - [x] **U14b-T1 (GREEN)** — Persisted immutable correction linkage and routed the freshly verified envelope through the existing guarded promotion to create `g+1` with new lineage/audit reference; ambiguity retains the catalog restriction. The U14b migration explicitly grants execute on the existing `promote_delta(JSONB)` boundary only to its internally required `ofertasuper_authority` session role.
    - [x] **U14b-T2 (TRIANGULATE)** — Disposable PostgreSQL proof passed for bad delta → unrelated successor → fresh correction, no selectable historical row, and unresolved overlap remaining restricted; the inherited EXIT trap removed the labelled container.
    - [x] **U14b-T3 (VERIFY)** — Focused lifecycle, executable PostgreSQL harness, full test, typecheck, lint, build, and diff checks passed without entering U15.
...
- **U14b closure:** The existing `promote_delta(JSONB)` SECURITY DEFINER boundary already rejects every `session_user` except `ofertasuper_authority`; U14b now grants execute only to that role, preserving the sole promotion mechanism. The direct disposable harness invocation passed after its executable fixture bit was restored, and its inherited EXIT trap left no labelled container. Focused lifecycle, PostgreSQL, full test (99 pass, 12 skips), typecheck, lint (0 errors, 206 warnings), build, and diff checks passed.
- **Next action:** U14 is complete. Do not enter U15 without separate authorization.
