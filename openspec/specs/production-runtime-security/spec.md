...
- AND the release candidate MAY enter runtime validation
...
- THEN the security closure MUST remain incomplete
...
The production runtime MUST preserve authenticated and unauthorized access decisions, public catalog availability, PWA installation, cache and offline fallback behavior, representative image rendering, production build success, and standalone startup. Each boundary MUST have production-like evidence before promotion.

#### Scenario: PRS-SCN-003 Authenticated and public continuity
- GIVEN a validated release candidate
...
- AND the release state MUST be recorded as incomplete or rolled back

### Requirement: Evidence handoff without cross-change completion
**Stable ID:** PRS-REQ-004

The system MUST produce traceable audit, runtime, rollout, and release-state evidence for `production-readiness` task 1.3. This handoff MUST NOT modify that task's completion state or claim its gate is complete; `production-readiness` SHALL evaluate and record its own gate.

#### Scenario: PRS-SCN-010 Evidence is handed off
- GIVEN the required audit, runtime, rollout, and release-state evidence is complete
...
- GIVEN required evidence is incomplete or the release is rolled back
