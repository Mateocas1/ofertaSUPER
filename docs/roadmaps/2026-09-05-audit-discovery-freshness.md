...
**Métricas mínimas:** `coverage (%) = 100 × |identidades cubiertas ∩ denominador observado| / |denominador observado|`; registrar contexto, ventana, exclusiones y si el recorrido quedó capped. Eso mide cobertura de lo observado, no del catálogo interno. Reportar también avance de enumeración y huecos conocidos.

`freshness_H(t) (%) = 100 × cantidad de observaciones elegibles con comprobación válida de edad < H / tamaño de la cohorte elegible declarada`. Congelar/versionar el denominador; no eliminar filas solo porque envejecieron o dejaron de rankear. Cero filas produce no-evaluable, no 100%.
