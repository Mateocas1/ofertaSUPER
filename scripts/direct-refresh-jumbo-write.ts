...
import {
	...
	type JumboActiveWriteCliOptions,
...
function createActiveWriteRepository(): ActiveWriteRepository {
	return {
		withTransaction: (fn) =>
			db.$transaction(
				async (tx) => fn(createJumboActiveWriteTransaction(tx)),
				DIRECT_REFRESH_ACTIVE_WRITE_TRANSACTION_OPTIONS,
			),
	};
}
export function createJumboActiveWriteTransaction(
	tx: Prisma.TransactionClient,
): ActiveWriteTransaction {
	return {
		async captureSourceDelta(capture) {
			if (capture.source !== "jumbo") throw new Error("Jumbo capture source policy mismatch");
...
async function writeJson(output: string, report: unknown) {
	...
	process.stdout.write(`Wrote Jumbo active refresh report to ${output}\n`);
}
...
async function buildFreshPrewrite(
	...
	return buildDirectRefreshPrewriteGate({
		...
		fetchDirectProducts: async (_sourceSlug, lookup) =>
			getSourceAdapter("jumbo").fetchDirectProducts(lookup),
	});
}

async function main() {
	const options = parseJumboActiveWriteCliOptions();
	...
	const report = await executeJumboActiveWrite({
