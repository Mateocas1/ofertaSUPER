...
function createActiveWriteRepository(): ActiveWriteRepository {
	return {
		withTransaction: (fn) =>
			db.$transaction(
				async (tx) => fn(createMasActiveWriteTransaction(tx)),
				DIRECT_REFRESH_ACTIVE_WRITE_TRANSACTION_OPTIONS,
			),
	};
}
export function createMasActiveWriteTransaction(
...
async function writeJson(output: string, report: unknown) {
	...
	process.stdout.write(`Wrote MAS active refresh report to ${output}\n`);
}
